...
<script type="text/javascript">
	var showUrl = "{{ route('patners.show',':id') }}";
	var updateUrl = "{{ route('patners.update','1') }}";
	var storeUrl = "{{ route('patners.store') }} ";
</script>
	<!-- BEGIN .app-main -->
	<div class="app-main">
							...
							<form method="POST" name="myForm" id="myForm"  enctype="multipart/form-data" onsubmit="return form_validation()">
					           ...
					           <div class="form-group row">
						            <div class="col-md-6">
						              ...
						              <input type="text"  name="link"  class="form-control-sm form-control">
						            </div>   
					        	</div>
					...
					<div class="card">
						<div class="card-header">Edit <button class='btn btn-primary btn-sm float-right' onclick="$('#editCard').hide()">Close</button></div>
							...
							<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
								<div class="card">
									<div class="card-header">patners <button class='btn btn-primary btn-sm float-right' onclick="addNew()">Add</button></div>
									<div class="card-body">
											...
											<?php foreach ($data as $row): ?>
											  ...
											  <tr id="row{{ $row->id }} ">
												...
												<td style="padding: 2px;text-align: center;" width="15%">
													...
													<form style="display: none;" id="{{ 'formno'.$row->id }}" action="{{ route('patners.destroy',$row->id) }}" method="POST">
														@method('DELETE')
				                                        @csrf
				                                    </form>
